...
def problem_two(request):
  instructors_hire = Instructor.objects.all().order_by("hire_date")
  for instructors in instructors_hire:
    print(f"Full name: {instructors.first_name} {instructors.last_name}\nHire Date: {instructors.hire_date}\n")
...
def problem_three(request):
  # instructor_courses = Instructor.objects.filter(pk=2)
  # courses = Course.objects.filter(instructor=2)
  # for instructor in instructor_courses:
  #   print(f"Instrtuctor name: {instructor.first_name} {instructor.last_name}.\nCourses:\n   - {courses}\n   - {courses}")
  courses = Course.objects.filter(pk=2)
  for course in courses:
    print(f"Instrtuctor name: {course.instructor.first_name} {course.instructor.last_name}.\nCourses:\n   - {course.name}\n   - {course.name}")
...
def problem_four(request):
  ...
  print(f"Student count: {count_of_students}\nCourses count: {count_of_courses}\nInstructors count: {count_of_instructors}")
  return complete(request)
...
def problem_five(request):
  new_student = Student.objects.create(first_name = "Kyle", last_name = "Hardwood", year = 2022, gpa = 3.0)
